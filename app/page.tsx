...
import type { HandlePaginationT } from "@/types/Pagination";
...
export default function Home() {
  ...
  const handleFilter = (
    ...
    section: Section,
    option: Brand | Category
  ) => {
    setFilter((prev) => ({
      ...prev,
      [section.id]: e.target.checked
        ? [...prev[section.id], option.value]
        : prev[section.id].filter((item) => item != option.value),
    }));
  };
  ...
  const handlePagination: HandlePaginationT = (e, page) => {
    e.preventDefault();
    if (page >= 1) {
      setPage(page);
    } else {
